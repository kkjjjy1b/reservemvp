...
- `DATABASE_URL`
- `SESSION_SECRET`
- `BLOB_READ_WRITE_TOKEN` (`/me` 프로필 이미지 업로드를 사용할 경우 필수)
- `RESEND_API_KEY` (예약 참여자 메일 발송을 사용할 경우 필수)
- `RESEND_FROM_EMAIL` (예약 참여자 메일 발송을 사용할 경우 필수)
- `RESERVATION_REPLY_TO_EMAIL` (선택)
- `APP_URL` (메일 본문에 서비스 링크를 넣을 경우 권장)

추가 운영값이 생기면 이 섹션에 확장한다.
현재 준비 상태와 보안 취급 원칙은 `docs/ops-env-checklist.md`에서 함께 관리한다.
현재 운영 결정상 메일 발송은 커스텀 도메인 확보 전까지 보류한다.
...
- 참여자 검색 가능
- 참여자 포함 예약 생성 / 수정 가능
- 참여자 예약 알림 메일 발송 가능 여부는 커스텀 도메인 확보 후 재검수
- 반복 예약은 현재 메일 발송 대상에서 제외됨
- 내 예약 목록 조회 가능
...
- 반복 예약은 기간 내 평일(`월~금`)만 생성
- 반복 예약은 최대 `30회`까지만 생성
- 반복 예약은 전체 사전 검증 후 하나라도 충돌하면 전체 실패
- 시작 시간이 지난 예약은 수정 불가
- 취소는 가능
